...
type ServiceTaskStateHandler struct {
	interceptors []process_ctrl.StateHandlerInterceptor
}
...
func (s *ServiceTaskStateHandler) Process(ctx context.Context, processContext process_ctrl.ProcessContext) error {
	...
	handleResultErr := func(err error) {
		...
		utils.HandleException(processContext, serviceTaskStateImpl.AbstractTaskState, err)
	}
...
func (s *ServiceTaskStateHandler) StateHandlerInterceptorList() []process_ctrl.StateHandlerInterceptor {
	return s.interceptors
}

func (s *ServiceTaskStateHandler) RegistryStateHandlerInterceptor(stateHandlerInterceptor process_ctrl.StateHandlerInterceptor) {
