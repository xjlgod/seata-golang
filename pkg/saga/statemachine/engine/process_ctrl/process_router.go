...
import (
	...
	"github.com/seata/seata-go/pkg/saga/statemachine/engine/process_ctrl/routers"
...
func (d *DefaultRouterHandler) matchProcessType(ctx context.Context, processContext ProcessContext) ProcessType {
	processType, ok := processContext.GetVariable(constant.VarNameProcessType).(ProcessType)
	if !ok || processType == "" {
		processType = StateLang
	}
	return processType
}

func (d *DefaultRouterHandler) EventPublisher() events.EventPublisher {
	return d.eventPublisher
}

func (d *DefaultRouterHandler) SetEventPublisher(eventPublisher events.EventPublisher) {
...
func (s *StateMachineProcessRouter) Route(ctx context.Context, processContext ProcessContext) (Instruction, error) {
	...
	if interceptors == nil || len(executedInterceptors) == 0 {
		...
		if instruction == nil && !stateInstruction.End() {
			err := utils.EndStateMachine(ctx, processContext)
			if err != nil {
				return nil, err
			}
		}
...
func (s *StateMachineProcessRouter) InitDefaultStateRouters() {
	if s.stateRouters == nil || len(s.stateRouters) == 0 {
		...
		taskStateRouter := &routers.TaskStateRouter{}
		...
		endStateRouter := &routers.EndStateRouter{}
